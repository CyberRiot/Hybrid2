...
int main() {
    try {
        ...
        std::string data_path = "E:\\Code\\0) Projects\\C++ Hybrid NN\\data\\output_binary_file.data";
        std::string label_path = "E:\\Code\\0) Projects\\C++ Hybrid NN\\data\\VOOD_labels.csv";
        std::string delimiter = ",";

        // Load data and labels
        dh.read_data(data_path);
        dh.read_labels(label_path, delimiter);

        // Split data
        dh.split_data();

        // Debugging output: Verify loaded data and labels
        auto data_array = dh.get_data_array();
        std::cout << "Loaded " << data_array->size() << " data samples." << std::endl;

        for (size_t i = 0; i < data_array->size(); ++i) {
            data* d = (*data_array)[i];
            std::cout << "Sample " << i + 1 << " | Feature Vector Size: " 
                      << d->get_feature_vector()->size() 
                      << " | Label: " << d->get_label() 
                      << " | Class: " << d->get_class_vector() 
                      << std::endl;
        }

        // Split sizes
        std::cout << "Training Data: " << dh.get_training_data()->size()
                  << ", Testing Data: " << dh.get_testing_data()->size()
                  << ", Validation Data: " << dh.get_validation_data()->size() 
                  << std::endl;
